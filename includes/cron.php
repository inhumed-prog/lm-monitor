...
function lm_monitor_run_checks() {
    ...
    try {
        ...
        foreach ($sites as $site) {
            try {
                $check_result = lm_monitor_check_and_update_site($site->id, $site->url);
                ...
                lm_monitor_log('Exception checking site ID ' . $site->id . ': ' . $e->getMessage(), 'error');
                $failed_count++;
            }
        }
...
function lm_monitor_manual_check_all() {
    ...
    foreach ($sites as $site) {
        $result = lm_monitor_check_and_update_site($site->id, $site->url);
